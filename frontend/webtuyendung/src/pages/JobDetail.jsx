...
const JobDetail = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (!allowed.includes(file.type)) { setError('Chỉ chấp nhận file .pdf, .doc, .docx'); setCv(null); return; }
    if (file.size > 5 * 1024 * 1024) { setError('File phải nhỏ hơn 5MB'); setCv(null); return; }
    setError(''); setCv(file);
  };

  const handleSubmit = async () => {
    if (!form.fullName || !form.email || !form.phone) { setError('Vui lòng điền đầy đủ thông tin'); return; }
    if (!cv) { setError('Vui lòng chọn file CV'); return; }
    ...
    try {
      ...
      fd.append('cv', cv);
      const res = await fetch(`${API_URL}/apply/${id}`, { method: 'POST', body: fd });
      const data = await res.json();
  ...
  const openApply = () => {
    if (!user) { navigate('/jobseeker/login'); return; }
    if (user.role !== 'ungvien') { alert('Chỉ ứng viên mới có thể ứng tuyển!'); return; }
    setSubmitted(false); setError(''); setCv(null);
    setForm({ fullName: user.name || '', email: user.email || '', phone: '' });
    setShowApply(true);
  };

  const closeModal = () => { setShowApply(false); setSubmitted(false); setError(''); };

  const tagStyle = (tag) => ({ 'GẤP': { bg: '#fff3e0', color: '#e65100' }, 'HOT': { bg: '#fce4ec', color: '#c62828' }, 'NỔI BẬT': { bg: '#e8f5e9', color: '#2e7d32' } }[tag] || null);
